...
This draft Enterprise Architecture portfolio report summarizes 8 applications, 6 technology records, 6 data assets, and 5 registered risks from the provided inventories. The application portfolio is concentrated in customer channels, integration, core banking, data/analytics, and fraud/security capabilities.

Facts: 2 applications are classified Critical and 6 are classified High. 7 applications process restricted data classifications, and 3 applications use cloud or hybrid deployment models. Business and technical owners are populated for all application records in the provided inventory.

Primary risks are ESB concentration, incomplete classification evidence, local/authentication exceptions, technology lifecycle uncertainty, and privacy definition gaps for the proposed Customer Onboarding App. Recommendations focus on resilience planning, lifecycle completion, authentication exception management, privacy controls, and inventory traceability.
...
|---|---|---|
| Critical | ESB, Core Banking | Material business disruption if unavailable; resilience, DR, monitoring, and recovery evidence should be explicit. |
| High | Mobile Banking, Internet Banking, API Gateway, Data Warehouse, Fraud Detection, Customer Onboarding App | High business or customer impact; availability objectives, support ownership, and incident procedures should be confirmed. |

## 5. Data Sensitivity

| Classification | Applications | Required Attention |
|---|---|---|
| Confidential - Token Metadata | API Gateway | Validate access controls, auditability, token/metadata handling, and monitoring requirements. |
| Restricted - Aggregated Customer Data | Data Warehouse | Validate privacy controls, access control, retention, auditability, and data-flow evidence. |
| Restricted - Customer PII | Mobile Banking, Internet Banking, Customer Onboarding App | Validate privacy controls, access control, retention, auditability, and data-flow evidence. |
| Restricted - Customer and Financial Data | Core Banking | Validate privacy controls, access control, retention, auditability, and data-flow evidence. |
| Restricted - Risk Data | Fraud Detection | Validate privacy controls, access control, retention, auditability, and data-flow evidence. |
| Restricted - Transactional Data | ESB | Validate privacy controls, access control, retention, auditability, and data-flow evidence. |

Data inventory facts: Restricted data assets are Customer Profile, Account Data, Transaction Data, and Fraud Risk Score. Confidential data assets are API Token Metadata and Portfolio Reporting Data. Retention is recorded as `Per policy`; exact retention durations are not provided.
...
| Internal Auth | Core Banking | Potential exception pattern; confirm approval, least privilege, rotation, monitoring, and migration/compensating controls. |
...
| Service Account | ESB, Fraud Detection | Potential exception pattern; confirm approval, least privilege, rotation, monitoring, and migration/compensating controls. |
...
| Technology: Mobile App + API | Mobile Banking | No matching technology inventory record found; confirm mapping to approved platform or add a technology record. |
| Technology: Web Application | Internet Banking | No matching technology inventory record found; confirm mapping to approved platform or add a technology record. |
| Technology: Web/Mobile | Customer Onboarding App | No matching technology inventory record found; confirm mapping to approved platform or add a technology record. |

Technology lifecycle gap: all technology inventory records have `version` and `eol_date` set to `TBD`, so lifecycle exposure cannot be fully assessed from the provided data.

Ownership gap facts: application business and technical owners are populated for all application records. Technology owner and data owner fields are also populated for the provided technology and data records. No missing owner field is evidenced, but formal accountability/RACI is not included.
...
|---|---|---|
| Reduce ESB concentration exposure | ESB; dependent transaction flows involving Fraud Detection and Data Warehouse | Risk register identifies multiple critical transaction flows depending on ESB availability. Resilience, failover, and flow-level dependency mapping should be prioritized. |
| Complete technology lifecycle baseline | API Gateway; ESB; Core Banking; Data Warehouse; OIDC Provider; Fraud Engine | All technology inventory records have TBD version and EOL values, limiting lifecycle and modernization decisions. |
| Strengthen authentication exception governance | ESB; Core Banking; Fraud Detection | Service Account and Internal Auth patterns require approval evidence, least-privilege validation, credential rotation, and migration or compensating controls. |
