...
**Generated:** 2026-05-18 17:05 Asia/Jakarta  
...
This draft report summarizes the application portfolio using the provided application, technology, data, and risk inventories. The portfolio contains 10 applications: 7 active and 3 proposed. Touchpoint Architecture has the largest application count, followed by Integration Architecture and Core Banking as listed in the inventory.

Key facts:

- 2 applications are marked Critical: ESB and Core Banking.
- 8 applications are marked High criticality.
- All application records include business owner and technical owner values.
- All application records include EA domain, deployment model, authentication pattern, status, and lifecycle stage.
- All application records involve Restricted or Confidential data classifications.
- Technology lifecycle information is incomplete: all listed technology versions and EOL dates are recorded as `TBD`.
- WMS and JF list technology as `Web`, which is not represented as a named technology in the technology inventory.
- The risk register contains 5 open risks, including ESB concentration risk, technology lifecycle uncertainty, local authentication exception, incomplete data classification, and onboarding privacy gaps.

Primary recommendations are to complete lifecycle data, validate authentication exceptions, reconcile newly proposed application classifications and technology mappings, define privacy and retention controls for proposed customer-facing applications, and establish resilience planning for critical integration dependencies.
...
| Proposed applications | 3: Customer Onboarding App; WMS; JF |
| Critical applications | 2: ESB; Core Banking |
| High criticality applications | 8 |
| EA domains represented | 6 as listed |
| Applications with business owner populated | 10 |
| Applications with technical owner populated | 10 |
| Applications with ownership gaps | 0 based on populated owner fields |
| Applications with Restricted data classification | 9 |
| Applications with Confidential data classification | 1 |
| Applications using SSO/OIDC, OIDC, or OIDC token validation | 6: Mobile Banking; Internet Banking; API Gateway; Customer Onboarding App; WMS; JF |
...
| On-prem / On-Prem applications | 7 |
...
| Technology mapping gaps | WMS and JF use `Web`, which is not listed in technology inventory |
...
| High | Mobile Banking; Internet Banking; API Gateway; Data Warehouse; Fraud Detection; Customer Onboarding App; WMS; JF | High criticality spans customer touchpoints, integration access, analytics, fraud screening, and proposed customer-facing applications. Authentication, data protection, deployment, technology mapping, and lifecycle controls should be validated before material change or expansion. |

## 5. Data Sensitivity

| Classification | Applications | Required Attention |
|---|---|---|
| Restricted - Customer PII | Mobile Banking; Internet Banking; Customer Onboarding App; WMS; JF | Validate privacy, consent, access, and retention controls. Proposed customer-facing applications should have documented data flow and retention evidence before approval. |
...
| Restricted - Customer and Financial Data | Core Banking | Maintain system-of-record governance, internal authentication controls, and data ownership alignment. |
...
| OIDC Token Validation | API Gateway | Gateway validates tokens and depends on identity platform and token metadata controls. |
| OIDC | Customer Onboarding App; WMS; JF | Proposed applications use OIDC. Privacy, retention, authorization, and data-flow evidence should be confirmed before approval. |
...
| API Gateway / On-prem | API Gateway | Active high-criticality integration control point; technology version and EOL are `TBD`. |
| ESB / Middleware / On-prem | ESB | Critical central integration layer; concentration risk is open. Technology version and EOL are `TBD`. |
| Core Banking Platform / On-prem | Core Banking | Critical system of record. Technology version and EOL are `TBD`. |
| Data Warehouse / Cloud | Data Warehouse | Active high-criticality analytics platform. Technology version and EOL are `TBD`. |
| Fraud Engine / On-prem | Fraud Detection | Active high-criticality fraud screening platform. Technology version and EOL are `TBD`. |
...
| Web / On-Prem | WMS; JF | Proposed high-criticality applications using Restricted customer PII. `Web` is not listed as a named technology in technology inventory. |
...
| Incomplete data classification | Medium | Portfolio-wide | Complete application and data inventory classification review; confirm classifications align between application and data inventories, including newly proposed WMS and JF. |
...
| Technology mapping gap | Medium | WMS; JF | Add or map `Web` technology to the technology inventory with owner, lifecycle, hosting, and EOL data. |
...
|---|---|---|
| Complete technology lifecycle baseline | API Gateway; ESB; Core Banking; Data Warehouse; OIDC Provider; Fraud Detection; WMS; JF | Listed technologies have `TBD` version and EOL data, and WMS/JF technology mapping is not represented in the technology inventory. |
...
| Standardize authentication governance | Core Banking; Data Warehouse; ESB; Fraud Detection; Customer Onboarding App; WMS; JF | Internal Auth, AD/LDAP, OIDC, and service account patterns require explicit governance, ownership, and access reviews. |
| Strengthen privacy-by-design for proposed customer-facing applications | Customer Onboarding App; WMS; JF | Proposed applications use Restricted customer PII and require confirmed consent, retention, authorization, and data flow controls. |
...
| High | Complete privacy, consent, retention, and data-flow definition for proposed customer-facing applications. | Digital Delivery Team | Reduces privacy and compliance risk before approval. |
| Medium | Add or reconcile technology inventory entries for `Web` used by WMS and JF. | Digital Delivery Team / Infrastructure Architecture | Improves technology ownership, lifecycle visibility, and deployment governance. |
...
- Is `Core Banking` intended to be an EA domain value for WMS and JF, or should these records map to an existing architecture domain?
- What technology inventory entry should represent `Web` for WMS and JF?
...
- What recovery time and recovery point objectives apply to ESB, Core Banking, API Gateway, Fraud Detection, WMS, and JF?
- Are service account rotation, vaulting, and monitoring controls documented for ESB and Fraud Detection?
- What consent, retention, and privacy data-flow requirements apply to the proposed Customer Onboarding App, WMS, and JF?
...
- Application names and EA domain values are used as listed in the inventory; no additional applications or domain mappings were inferred.
- `On-prem` and `On-Prem` deployment values are counted together for deployment observations while preserving listed values in detailed tables.
